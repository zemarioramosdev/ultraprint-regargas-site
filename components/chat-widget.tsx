...
export function ChatWidget() {
  ...
  const sendToN8n = async (userMessage: string): Promise<string> => {
    if (!N8N_WEBHOOK_URL) {
      // Respostas locais quando n8n não está configurado
      ...
      if (lowerMessage.includes("endereço") || lowerMessage.includes("endereco") || lowerMessage.includes("localização") || lowerMessage.includes("localizacao")) {
        return "Estamos localizados na:\n\nRua das Impressoras, 123 - Centro\nBelo Horizonte - MG\nCEP: 30000-000\n\nFácil estacionamento na região!"
      }
    ...
    try {
      ...
      return "Desculpe, estou com dificuldades técnicas no momento. Por favor, entre em contato pelo WhatsApp para um atendimento imediato."
    }
  }
